...
<template>
  <div class="mx-auto max-w-4xl px-4 py-8">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800">簽核管理</h1>
      <span class="text-sm text-gray-500">{{ store.pendingApprovals.length }} 筆待簽核</span>
    </div>
    ...
    <!-- Empty State -->
    <div
      v-else-if="store.pendingApprovals.length === 0"
    ...
    <div v-else class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr
            v-for="record in store.pendingApprovals"
            :key="record.id"
